...
set ([handles.axes_mag, handles.showplot_h.axes_mag], ...
    'XLim', [-pi,pi]*0.5/pi, 'YLim', [min([-1,abs(handles.hz)])*1.2, max([2,abs(handles.hz)])*1.2], ...
    'FontName','Symbol','Xtick',[-pi -pi/2 0 pi/2 pi]*0.5/pi,'XTickLabel',{'-1/2';'-1/4';'0';'1/4';'1/2'});
...
set ([handles.axes_phase, handles.showplot_h.axes_phase], ...
    'XLim', [-pi,pi]*0.5/pi, 'YLim', [min([-pi,angle(handles.hz)])*1.2, max([pi,angle(handles.hz)])*1.2], ...
    'FontName','Symbol','Xtick',[-pi -pi/2 0 pi/2 pi]*0.5/pi,'XTickLabel',{'-1/2';'-1/4';'0';'1/4';'1/2'} );

set ([handles.linemag, handles.showplot_h.linemag], ...
    'XData', [-pi : 2*pi/length(abs(handles.hz)) : pi-2*pi/length(abs(handles.hz))]*0.5/pi, ...
