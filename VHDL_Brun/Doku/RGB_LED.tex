...
Die beiden Bauteile, welche betroffen sind, sind der Stromregler und die RGB LED. Der Stromregler hat jedoch im Datenblatt nur Wärmewiderstandsangaben für eine Printmontage bei der die Erdungsfahne auf einer mindestens $2Inch^2$ grossen Kupferfläche angelötet ist. Aufgrund des guten Wirkungsgrades des Reglers haben wir vermutet, dass keine spezielle Kühlung nötig ist. Dies hat sich schlussendlich auch nach mehreren Testdurchgängen bestätigt.
...
Nun wollen wir wissen, wie tief der Wärmewiderstand $R_{\vartheta KA}$ des Kühlkörpers an die Umgebung sein muss, um bei einer Umgebungstemperatur von $25^\circ C$ eine Halbleitertemperatur von höchstens $125^\circ C$ zu erreichen. Wir rechnen dabei mit der maximalen Leistung der RGB LED von $40W$.
...
Bei Farnell haben wir einen Kühlkörper gefunden mit einem Wärmewiderstand von $R_{\vartheta KA} = 0.7K/W$, welcher nur $60x60mm$ gross ist (\autoref{fig:Kuehlkoerper}).

\begin{figure}[ht]
	\centering
		\includegraphics[width=0.30\textwidth]{images/Kuehlkoerper.png}
	\caption[Kühlkörper für RGB LED \protect{\small\url{http://ch.farnell.com} (18.\,06.~2013)}]{Kühlkörper für RGB LED}
	\label{fig:Kuehlkoerper}
\end{figure}

Dieser hat einen besseren Wer als erwartet und wir werden voraussichtlich höchstens folgende Halbleitertemperatur erreichen:
...
Dies ist zwar immer noch recht hoch, wird aber in der Praxis höchst wahrscheinlich nie vorkommen, da die Farbe Weiss mit unserer ganzen Vorschaltung und HSV-RGB Umwandlung relativ uninteressant ist.

\subsection{Hardwareaufbau}
Um unsere Hardware zu testen und zu schauen, ob die Stromregler überhaupt funktionieren, haben wir zuerst ein Aufbau auf einer Steckplatte realisiert (siehe \autoref{fig:Aufbau Steckplatte}) und erst danach noch einen Prototypaufbau auf einer Veroboardplatte (siehe \autoref{fig:Aufbau Veroboard}) gemacht.
\subsubsection{Steckplatte}
Um zu verhindern, dass die LED beim ersten Einschalten zerstört wird, haben wir den Widerstand $R_{SET}$ zuerst $1k\Omega$ gewählt, was einen Strom durch den Regler von etwa $500mA$ ergibt.
\begin{figure}[ht]
	\centering
...
Als dies funktioneirte, haben wir den Widerstand mit einem $680\Omega$ Widerstand ersetzt, so dass der Strom nun auf 800mA eingestellt ist. Diesen Wert haben wir auch auf dem Veroboardaufbau eingesetzt. Jedoch haben wir natürlich aus Neugier die LED auch einmal mit einem Ampere pro Farbe betrieben. Der Unterschied von $800mA$ zu $1A$ war doch recht deutlich zu sehen. Da wir aber die gesamte Schaltung auf $800mA$ ausgelegt haben, haben wir schlussendlich den Widerstand nicht mer definitiv ersetzt auf dem Prototypaufbau.

\subsubsection{Veroboard}
Nach dem Testaufbau auf der Steckplatte folgte ein Prototypaufbau auf einem Veroboard. Das hat den Vorteil, dass die Schaltung übersichtlicher gestaltet ist und der Aufbau insgesamt etwas stabiler und zuverlässiger ist. Auch sind nun die Verbindungen gelötet und nicht nur gesteckt, was bei den relativ hohen Strömen auch ein Vorteil ist.
\begin{figure}[ht]
	\centering
...
\end{figure}
Auf dem Board sind im Wesentlichen nur die drei Stromregler und die dazugehörigen Widerstände. Dazu noch die Stecker um die Netzgeräte, das Spartan Board und die LED anzuschliessen. Das genaue Anschlussschema ist im Kapitel Inbetriebnahme ersichtlich und wird hier nicht weiter erklärt. 

\section{Inbetriebnahme}
Um die LED in Betrieb zu nehmen, haben wir zwei Labornetzgeräte vom Typ MN 300-2 benutzt und die Spannungen $V_{Rot} = 10.5V$, $V_{Gruen} = 17.5$, $V_{Blau} = 14.5V$ und $V_{CC} = 5V$ eingestellt. Wie die Spannungen angeschlossen werden, ist im Anschlussschema (\autoref{fig:Anschlussscehma}) ersichtlich.
\begin{figure}[ht]
	\centering
		\includegraphics[width=1\textwidth]{images/Anschlussschema.pdf}
	\caption[Anschlussschema]{Anschlussschema}
	\label{fig:Anschlussscehma}
\end{figure}
Die Strombegrenzung haben wir dabei zuerst bei etwa $500mA$ eingestellt, kann aber bei der jetzigen Inbetriebnahme auf ein Ampere aufgedreht werden. 
Um sicherzugehen, dass die digitale Ansteuerung funktioniert, haben wir das PWM Signal zuerst mit einem Oscilloskop ausgemessen. Auch den Prototypaufbau haben wir zuerst kontrolliert, indem wir gemessen haben, ob wir keinen Kurzschluss auf den Verschiedenen Speisungen haben. Als alles kontrolliert war, haben wir die LED an das Interface angeschlossen und geprüft, ob die LED's mit dem Stromregler angesteuert werden können. Nach erfolgreicher Ausführung dieses Schritts, schlossen wir das Spartan Board an, und siehe da - Es funktioniert!  

Zu beachten gibt es noch, dass das Flachbandkabel, welches auf das Spartan Board fürht richtig angeschlossen wird. Auf dem Spartan Board sind nur einreihige Header Steckerliesten vorhanden. Wir haben einen Adapter für ein Flachbandkabel gemacht und können somit das Interface direkt anschliessen.

\section{Schlussfolgerung}
Die Projektarbeit hat uns gut gefallen, besonders gut war dabei, dass wir den analogen und den digitalen Teil verbinden konnten. Wie sich im Verlauf der Arbeit herausgestellt hat, war vorallem die Ansteuerung im VHDL der LED die grosse Herausforderung. Wir haben zuerst sehr viel Zeit mit dem VHDL Entwurf verbracht und den analogen Teil etwas vernachlässigt. Desshalb hatten wir am Schluss noch etwas Stress beim fertign des Prototyps. 
Grössere Probleme gab es eigentlich keine. Wir hatten jedoch immer noch etwas mühe, im VHDL nicht prozeduralen Code zu schreiben, sondern nur Hardware zu beschreiben. Mit den Stromreglern hatten wir noch kleine Schwieregkeiten. Es schaltete immer ein Stromregler in den Shutdown Mode, und funktionierte nach einer gewissen Zeit nicht mehr. Nach längerem Fehlersuchen, haben wir den Regler ersetzt und alle Farben zu Beginn gleich initialisiert. So konnten wir das Problem beheben.
