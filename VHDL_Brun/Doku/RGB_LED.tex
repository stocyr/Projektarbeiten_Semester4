...
Nach dem Testaufbau auf der Steckplatte folgte ein Prototyp-Aufbau auf einem Veroboard. Das hat den Vorteil, dass die Schaltung übersichtlicher gestaltet ist und der Aufbau insgesamt etwas stabiler und zuverlässiger ist. Auch sind nun die Verbindungen gelötet und nicht nur gesteckt, was bei den relativ hohen Strömen auch ein Vorteil ist.
\begin{figure}[ht]
	\centering
...
\end{figure}
Auf dem Board sind im Wesentlichen nur die drei Stromregler und die dazugehörigen Widerstände. Dazu noch die Stecker um die Netzgeräte, das Spartan 3E Board und die LED anzuschliessen. Das genaue Anschlussschema ist in \autoref{sec:Inbetriebnahme} ersichtlich. 

\section{Inbetriebnahme}
\label{sec:Inbetriebnahme}
Um die LED in Betrieb zu nehmen, haben wir zwei Labornetzgeräte vom Typ \emph{MN 300-2} benutzt und die Spannungen $V_{Rot} = 10.5V$, $V_{Gruen} = 17.5$, $V_{Blau} = 14.5V$ und $V_{CC} = 5V$ eingestellt. Wie die Spannungen angeschlossen werden, ist im Anschlussschema (\autoref{fig:Anschlussscehma}) ersichtlich.
\begin{figure}[ht]
	\centering
...
\end{figure}
Die Strombegrenzung haben wir dabei vorerst bei etwa $800mA$ belassen. Diese könnte aber bei der jetzigen Inbetriebnahme auf ein Ampere aufgedreht werden. 
...
Zu beachten gilt noch, dass das Flachbandkabel, welches auf das Spartan 3E Board führt, richtig angeschlossen wird. Auf dem Spartan Board sind nur einreihige Header-Steckerleisten vorhanden. Wir haben einen Adapter für ein Flachbandkabel gemacht und können somit das Interface direkt anschliessen.

\section{Schlussfolgerung}
Die Projektarbeit hat uns gut gefallen. Besonders gut war dabei, dass wir den analogen und den digitalen Teil verbinden konnten. Wie sich im Verlauf der Arbeit herausgestellt hat, war vor allem die Ansteuerung der LED in VHDL die grosse Herausforderung. Wir haben zuerst sehr viel Zeit mit dem VHDL Entwurf verbracht und den analogen Teil etwas vernachlässigt. Deshalb hatten wir am Schluss noch etwas Stress beim Fertigen des Prototyps. 
Grössere Probleme gab es eigentlich keine. Wir hatten jedoch immer noch etwas mühe, in VHDL \emph{nicht} prozeduralen Code zu schreiben, sondern nur Hardware zu beschreiben. Mit den Stromreglern hatten wir noch kleine Schwierigkeiten. Es schaltete immer ein Stromregler in den Shutdown Mode und funktionierte nach einer gewissen Zeit nicht mehr. Nach längerem Fehlersuchen haben wir den Regler ersetzt und alle Farben zu Beginn gleich initialisiert, und zwar alle auf dunkel. So konnten wir das Problem beheben.
